...
class ResConvINormConv(nn.Module):
    def __init__(self, in_channels, kernel_size=3):
        super().__init__()
        self.in_channels = in_channels
        self.conv1 = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, padding="same", bias=False)
        self.norm1 = nn.InstanceNorm3d(in_channels)
        self.activation = nn.LeakyReLU(inplace=True)
        self.conv2 = nn.Conv3d(in_channels, in_channels, kernel_size=kernel_size, padding="same", bias=False)
...
class ResDownArm(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.in_channels = in_channels
        self.down_sample = nn.Conv3d(in_channels, out_channels, kernel_size=3, padding=1, stride=2)
        self.conv = ResConvINormConv(out_channels)
...
class ResUpArm(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.in_channels = in_channels
        self.up_sample = nn.Upsample(scale_factor=2, mode="trilinear", align_corners=True)
        self.combine = nn.Conv3d(in_channels, out_channels, kernel_size=1)
        self.conv = ResConvINormConv(out_channels)
